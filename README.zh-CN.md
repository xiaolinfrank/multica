...
BayClaw 大湾区部署运行一个算力池：协调节点（运行 Multica 服务）加上局域网内的
Mac 工作节点（`fosun_agent_1..6`）。实时系统状态（CPU / 内存 / 磁盘 / 负载 /
Docker / 运行时长）在应用内左侧 **算力池 / Fleet** 菜单查看，由 `GET /api/fleet/status`
提供数据——后端从服务器主机通过 SSH 采集各节点。

设备清单、幂等环境配置脚本（Homebrew + Colima + Docker）、健康检查脚本与运维 skill
均在 [`deploy/fleet/`](deploy/fleet/README.md)。
