...
智能体默认最多同时执行 6 条执行任务；单个守护进程默认最多同时执行 20 条。达到上限时，新执行任务会留在队列中，等已有执行结束后再开始。运行时离线时，执行任务也会继续排队；超过 2 小时（默认值）仍未领取才会失败（上限可用 `MULTICA_TASK_QUEUED_TTL` 调整，见[环境变量](/environment-variables)）。
