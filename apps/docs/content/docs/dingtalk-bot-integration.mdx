...
Send `/new`, and the next message sent successfully will be processed by the agent in a fresh context. You can also send `/new <message>` to have the agent process that message immediately in a fresh context.

The `/new` command only makes the agent process messages in a fresh context. It does not create a new Multica Chat.
