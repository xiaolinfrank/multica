...
Add up to three **suggested first actions** to show people what this agent is good at before they send a first chat message. Each suggestion has a short label and a complete prompt. Selecting one fills the composer so the user can review or edit it; it never starts a run automatically.

If you leave the list empty, Chat shows localized general-purpose suggestions. Agent-specific examples are usually more useful because they can reflect the role and boundaries in the instructions.
...
Duplicating carries over most working configuration except the name, including instructions, suggested first actions, skills, custom arguments, avatar, concurrency limit, and Access settings. The model, thinking level, and service tier come along too; if the original runtime is unavailable and the copy is forced onto another runtime, those three are cleared and must be re-selected.
