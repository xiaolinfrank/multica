...
export const WORKSPACE_PAGES: Record<WorkspacePageKey, WorkspacePage> = {
  ...
  // table is the single source for the sidebar icon and the desktop tab title,
  // and an unregistered segment silently falls back to the Issues icon and a
  // "unknown" tab label without failing any type check.
  fleet: { segment: "fleet", icon: "Server", navKey: "fleet" },
  env: { segment: "env", icon: "KeyRound", navKey: "environment" },
  workspaces: { segment: "workspaces", icon: "FolderGit2", navKey: "workspaces" },
