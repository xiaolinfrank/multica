...
const WORKSPACE_ROUTES: readonly RoutePattern[] = [
  ...
  // BayClaw-only pages. Unregistered routes collapse to the `/:slug/*` mask,
  // so diagnostics could not tell these screens apart — see the "no builder
  // falls back to the mask" test.
  ["fleet"],
  ["env"],
  ["workspaces"],
];
