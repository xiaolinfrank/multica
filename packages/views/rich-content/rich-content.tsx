...
function RichLink({ href, children }: { href?: string; children?: ReactNode }) {
  ...
  // Must be checked before the regular-link branch: openLink treats any
  // "/"-prefixed href as an in-app route, so the link would dispatch a
  // navigation that matches nothing and clicking would do nothing at all. The
  // <Attachment> renderer resolves the id through the surrounding
  // AttachmentDownloadProvider and renders a real download/preview card. This
  // is how agents surface produced files (CSVs, plots, reports) into a comment
  // — see the guard in editor/readonly-content.test.tsx.
  if (href && attachmentIdFromDownloadURL(href)) {
    return (
      <AttachmentRenderer
        attachment={{ kind: "url", url: href, filename: childrenToLabel(children) ?? "" }}
      />
    );
  }
