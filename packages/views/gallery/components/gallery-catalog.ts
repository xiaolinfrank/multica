    ...
    diagrams: [
      {
        ...
        shortTitle: "系统集成架构",
        caption:
          "用户接入在上、运维支撑在下，中心云集成层里智能体平台之下并置独立的生物信息分析后端承接重型计算，"
          + "右侧是医院边缘可信数据空间；页脚串起从提问到报告交付的完整链路。",
        description:
          "四层手绘架构图。上层为用户接入层，科研用户端与运营管理端汇入统一 API 网关；"
          + "中层左侧为中心云平台集成层，智能体平台连接模型管理平台、算力调度平台、数据平台与用户虚拟资源空间，"
          + "其下部并置独立的生物信息分析后端（生信流程引擎 · Nextflow · 环境镜像仓库 · 容器化），"
          + "与智能体平台以两支编号箭头直连：投送分析环境与代码、回传状态与结果；"
          + "中层右侧为医院边缘节点可信数据空间，含安全通道网关、隐私计算引擎与不出院的原始医疗数据；"
          + "下层为运维支撑层。底部标注完整链路：提问、编排拆解、开沙箱分资源、生信任务投送、执行与状态结果回传、"
          + "敏感任务下发、结果回流、报告交付。",
      },
      {
        id: "data-lifecycle",
        ...
        caption:
          ...
          + "平台侧新增生物信息分析后端承接生信镜像构建与重型分析，原始层永不放行。",
        description:
          "数据全生命周期手绘图。左列为医院侧一体机，自上而下是原始数据源、数据治理流水线、本地存储、"
          + "院端连接器沙箱与可信连接器；右列为平台侧中心云，自上而下是中心侧连接器、数据平台、运营管理端、"
          + "智能体平台、生物信息分析后端与用户虚拟资源空间，智能体平台与后端之间以分析需求投送、状态结果回传"
          + "两支箭头构成协作环。两列之间标注上行策略：非敏感数据走目录加实体，敏感数据仅走目录加元数据。"
          + "底部并列三级数据分层：原始层永不放行、分析就绪层编目上行、群体统计量可上行；"
          + "页脚标注三类沙箱的执行位置：院端沙箱、中心沙箱（生物信息分析后端流水线）与用户虚拟资源空间，"
          + "生信环境镜像由后端统一构建、Nextflow 分发、随沙箱销毁。",
      },
      {
        id: "runtime",
        ...
        shortTitle: "技术流转",
        caption:
          "七条泳道、九个步骤：提问经意图拆解与资源预估落到用户空间，生信任务投送至生物信息分析后端执行，"
          + "执行期按计算、模型、敏感三路分发，敏感步骤只走 DSEP 合约。",
        description:
          "七条泳道的手绘流程图，自上而下为用户统一门户、智能体平台、生物信息分析后端、模型管理平台、数据平台、"
          + "运维平台用户空间、院端可信数据空间。九个步骤依次为科研提问、意图识别与拆解、资源预估、开用户空间、"
          + "资产检索、生成计划 DAG、拉起生信流水线（位于后端泳道，投送分析环境与代码）、执行期三路分发、报告交付；"
          + "计算步骤投送生物信息分析后端执行、产物写入用户 PVC，模型步骤走推理调用，"
          + "敏感步骤走 DSEP 合约投送院端、原始数据不出域。",
      },
    ],
  },
];
