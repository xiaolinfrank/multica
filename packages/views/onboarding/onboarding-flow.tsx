  ...
  return (
    ...
    >
      ...
      {/* Step 3 has two paths:
            - Desktop (no runtimeInstructions slot) drives the local daemon's
              runtime list directly.
            - Web (BayClaw is server-centric) → StepPlatformFork lists the
              platform-operated shared runtimes; nothing to install, and the
              runtimeInstructions slot is reused only as the web signal. */}
      {step === "runtime" &&
        workspace &&
        (!runtimeInstructions ? (
          <StepRuntimeConnect
