...
import { INSTALL_RUNTIME_ISSUE_TITLE } from "../onboarding/templates/install-runtime-issue";
import {
  NavigationProvider,
...
describe("WelcomeAfterOnboarding", () => {
  ...
  it("provisions the no-runtime guide, then opens the completion modal", async () => {
    ...
    expect(mockCreateIssue.mock.calls[0]![0]).toMatchObject({
      title: INSTALL_RUNTIME_ISSUE_TITLE.en,
      status: "in_progress",
  ...
  it("provisions the guide issue when the member retries", async () => {
    ...
    expect(await screen.findByText(/Welcome to BayClaw/i)).toBeInTheDocument();
