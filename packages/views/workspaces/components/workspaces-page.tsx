...
import { agentWorkspacesOptions, workspaceKeys } from "@multica/core/workspace";
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {groups.map((g) => (
              <IssueGroupRow key={g.issueId || g.identifier} wsId={wsId} group={g} />
            ))}
          </div>
        )}
      </div>
...
function IssueGroupRow({ wsId, group }: { wsId: string; group: IssueGroup }) {
  const { t } = useT("workspaces");
  ...
  return (
    <div className="rounded-lg border border-border bg-card">
      <div className="flex items-center gap-3 border-b border-border px-4 py-2.5">
        ...
        <DeleteIssueButton wsId={wsId} group={group} />
      </div>
      <ul className="divide-y divide-border">
        {group.agents.map((ws) => (
          <AgentRow key={ws.task_short} wsId={wsId} ws={ws} />
        ))}
      </ul>
    </div>
  );
...
function AgentRow({ wsId, ws }: { wsId: string; ws: AgentWorkspace }) {
  const { t } = useT("workspaces");
  const qc = useQueryClient();
  const [browsing, setBrowsing] = useState(false);

  const reclaim = useMutation({
    mutationFn: () => api.reclaimWorkspace(wsId, ws.task_short, "artifacts"),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: workspaceKeys.agentWorkspaces(wsId) });
      qc.invalidateQueries({ queryKey: workspaceKeys.workspaceTree(wsId, ws.task_short) });
    },
  });
...
function DeleteIssueButton({ wsId, group }: { wsId: string; group: IssueGroup }) {
  const { t } = useT("workspaces");
  const qc = useQueryClient();
  const [confirming, setConfirming] = useState(false);
  const del = useDeleteIssue();

  if (!group.issueId) return null;
      ...
      <AlertDialog open={confirming} onOpenChange={setConfirming}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t(($) => $.delete.body, {
                issue: group.identifier || group.title,
