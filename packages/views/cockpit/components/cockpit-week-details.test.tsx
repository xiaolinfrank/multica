...
it("wires progress, current progress, deliverables, linked issues and weekly payments even when tasks are collapsed", async () => {
  ...
  const board: CockpitBoard = { cockpit: { id: "cp", workspace_id: "ws", title: "Board", goal_title: "Goal", goal_date: "2026-12-31", summary_overall: "", summary_next: "", summary_support: "", basis: "", meeting_assignee_type: "", meeting_assignee_id: null, meeting_project_id: null, meeting_module_id: null, meeting_node_id: null, meeting_dir: "", created_at: "", updated_at: "" }, nodes: [base, { ...base, id: "task", parent_id: "root", code: "L3", name: "Validation", start_date: "2026-09-14", end_date: "2026-09-20", progress: 42, current_progress: "Reviewing evidence", deliverable: "Validated dataset" }], payments: [{ id: "pay", node_id: "task", label: "Acceptance payment", pay_date: "2026-09-16", amount: 25, position: 0 }, { id: "later", node_id: "task", label: "Later payment", pay_date: "2026-09-21", amount: 10, position: 1 }], issue_links: [{ id: "link", node_id: "task", issue_id: "issue", issue_number: 42, issue_identifier: "BIO-42", issue_title: "Evidence review", issue_status: "In progress", position: 0 }], milestones: [], meetings: [], meeting_issues: [], meeting_nodes: [] };
  const select = vi.fn();
  render(<I18nProvider locale="en" resources={{ en: { cockpit: enCockpit } }}><CockpitGantt board={board} today="2026-09-18" zoom="week" query="" rootIds={new Set()} collapsed={new Set(["root"])} onToggleCollapse={vi.fn()} onSelect={select} selectedId={null} onPatchNode={vi.fn()} statusSuggestions={[]} ownerSuggestions={[]} showFinance={false} toolbarOpen scrollToTodayNonce={0} focusTarget={null} /></I18nProvider>);
  ...
  expect(dialog.getByText("2026-09-16 · 25万")).toBeInTheDocument();
