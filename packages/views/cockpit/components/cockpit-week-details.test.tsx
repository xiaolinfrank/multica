...
it("wires progress, current progress, deliverables, linked issues and weekly payments even when tasks are collapsed", async () => {
  ...
  const select = vi.fn();
  render(<I18nProvider locale="en" resources={{ en: { cockpit: enCockpit } }}><CockpitGantt board={board} today="2026-09-18" zoom="week" query="" rootIds={new Set()} collapsed={new Set(["root"])} onToggleCollapse={vi.fn()} onSelect={select} selectedId={null} onPatchNode={vi.fn()} statusSuggestions={[]} showFinance={false} toolbarOpen scrollToTodayNonce={0} focusTarget={null} /></I18nProvider>);
