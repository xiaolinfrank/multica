...
var legacyDuplicateMigrationStems = map[string][]string{
	...
	// 127_workspace_shared_env is a BayClaw fork migration (workspace-level shared
	// environment variables). It is already applied in production under this exact
	// stem, and the migration runner keys schema_migrations on the full stem with no
	// checksum, so renaming it would replay a non-idempotent ALTER TABLE ADD COLUMN
	// and fail the backend's startup migration step. Keep the stem frozen and carry
	// this entry across upstream syncs instead.
	"127": {"127_issue_pull_request_reference_only", "127_task_squad_id", "127_user_composio_connection", "127_workspace_shared_env"},
