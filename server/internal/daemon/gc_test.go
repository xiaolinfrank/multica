...
func TestRunGC_MixedLayoutsUseMetadataWorkspaceIdentity(t *testing.T) {
	...
	// Fork policy: a done issue's workspace persists regardless of age (see
	// TestShouldCleanTaskDir_DoneIssuePersists). What this test locks in is the
	// metadata-driven workspace identity — both physical layouts resolve to the
	// same workspace and drive one batch request each — not reclamation.
	for _, taskDir := range []string{legacyDir, readableDir} {
		if _, err := os.Stat(taskDir); err != nil {
			t.Fatalf("task dir %s did not survive GC: %v (persistent workspace policy)", taskDir, err)
		}
	}
}
...
// TestGCDecisionIssueResult_PersistentWorkspacePolicy pins the persistent
// workspace policy directly on gcDecisionIssueResult, which is what the batch
// scanner (gcWorkspaceIssues) calls — the HTTP-path tests above only reach it
// through gcDecisionIssue. Upstream ships a "done/cancelled older than GCTTL
// -> gcActionClean" branch in this function; re-adopting it on an upstream sync
// would silently delete live agent workspaces, and because the batch scanner
// evaluates every candidate in a workspace in one pass, the blast radius is a
// whole workspace rather than a single directory. Nothing else fails if that
// branch comes back, so this test is the guard.
func TestGCDecisionIssueResult_PersistentWorkspacePolicy(t *testing.T) {
	t.Parallel()
	d := newGCTestDaemon(t, http.NewServeMux())
	issueID := "44444444-4444-4444-4444-444444444444"
	// CompletedAt now → the artifact-prune branch below the policy stays shut,
	// so gcActionSkip isolates the policy decision itself.
	meta := &execenv.GCMeta{IssueID: issueID, WorkspaceID: "ws1", CompletedAt: time.Now()}
	taskDir := createTaskDir(t, d.cfg.WorkspacesRoot, "ws1", "persist", meta)
	...
	for _, status := range []string{"done", "cancelled"} {
		result := IssueGCCheckResult{
			...
			Found:     true,
			Status:    status,
			UpdatedAt: time.Now().Add(-30 * 24 * time.Hour), // far past GCTTL
		}
		if action := d.gcDecisionIssueResult(taskDir, meta, result); action != gcActionSkip {
			t.Fatalf("status %q closed 30 days ago: expected gcActionSkip (workspace persists with its issue), got %d", status, action)
		}
	}

	// The counterpart the policy does keep: a deleted issue (Found=false) still
	// falls through to the mtime-gated orphan path rather than persisting forever.
	deleted := IssueGCCheckResult{ID: issueID, Found: false}
	if action := d.gcDecisionIssueResult(taskDir, meta, deleted); action != gcActionSkip {
		t.Fatalf("deleted issue with a fresh directory: expected gcActionSkip from orphanByMTime, got %d", action)
	}
}
...
func TestGCWorkspace_BatchesAndDeduplicatesIssueChecks(t *testing.T) {
	...
	// Give the two done dirs something reclaimable so the batch path still has
	// observable work under the persistent-workspace policy (artifacts go, the
	// workspace itself stays).
	...
	// The subject of this test: one batched POST covering both issues, the
	// duplicate issue id collapsed, and no fan-out to the legacy per-issue
	// endpoint.
	if batchRequests != 1 || legacyRequests != 0 {
		t.Fatalf("requests: batch=%d legacy=%d, want batch=1 legacy=0", batchRequests, legacyRequests)
	}
	// Persistent workspace policy: closing an issue never removes its
	// workspace, however long ago it closed. Upstream's version of this test
	// asserted the opposite; see gcDecisionIssueResult.
	for _, dir := range []string{doneA, doneB, open} {
		if _, err := os.Stat(dir); err != nil {
			t.Fatalf("task dir %s must persist: %v", dir, err)
		}
	}
	for _, dir := range []string{doneA, doneB} {
		if _, err := os.Stat(filepath.Join(dir, "node_modules")); !os.IsNotExist(err) {
			t.Fatalf("node_modules under %s should have been reclaimed", dir)
		}
	}
	if stats.cleaned != 0 {
		t.Fatalf("stats.cleaned = %d, want 0 (no whole-workspace removal under the persistent policy)", stats.cleaned)
	}
	if stats.artifactDirs != 2 {
		t.Fatalf("stats.artifactDirs = %d, want 2 (both done dirs pruned via the batch result)", stats.artifactDirs)
	}
}
...
func TestGCWorkspace_OldServerFallbackIsCached(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		if err := os.MkdirAll(filepath.Join(taskDir, "node_modules", "pkg"), 0o755); err != nil {
			t.Fatal(err)
		}
		d.gcWorkspace(context.Background(), wsDir, &gcStats{byPattern: map[string]int{}})
		// Persistent workspace policy: the fallback must still reach a decision
		// for this issue (proven by the artifact prune below), but a done issue
		// never costs its workspace.
		if _, err := os.Stat(taskDir); err != nil {
			t.Fatalf("task dir %s must persist: %v", taskDir, err)
		}
		if _, err := os.Stat(filepath.Join(taskDir, "node_modules")); !os.IsNotExist(err) {
			t.Fatalf("legacy fallback did not prune artifacts under %s", taskDir)
		}
	}
