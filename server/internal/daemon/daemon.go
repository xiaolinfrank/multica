...
func (d *Daemon) Run(ctx context.Context) error {
	...
	go d.gcLoop(ctx)
	go d.workspaceInventoryLoop(ctx)
...
func (d *Daemon) runRuntimeHeartbeat(ctx context.Context, rid string) {
	...
	tick := func() {
		if d.runHeartbeatTick(ctx, rid, false) {
...
func (d *Daemon) runHeartbeatTick(ctx context.Context, rid string, force bool) bool {
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	taskCtx := execenv.TaskContextForEnv{
		...
		ProjectCollabPath:                task.ProjectCollabPath,
	...
	// Both user-supplied env layers, not custom_env alone: HERMES_HOME and the
	// ${VAR} expansion in Hermes external_dirs must see keys that were set once
	// at the workspace level (shared_env) as well as per-agent ones, matching
	// what the agent process actually gets at launch.
	var agentEnvOverrides map[string]string
	var agentCustomArgs []string
	if task.Agent != nil {
		agentEnvOverrides = mergeUserEnvLayers(task.Agent.WorkspaceEnv, task.Agent.CustomEnv)
	...
	var agentWorkspaceEnv, agentCustomEnv map[string]string
	if task.Agent != nil {
		agentWorkspaceEnv = task.Agent.WorkspaceEnv
		agentCustomEnv = task.Agent.CustomEnv
	}
	applyUserEnvLayer(agentEnv, "workspace_env", agentWorkspaceEnv, d.logger)
	...
	// Codex runs shell tools in a child process whose env is filtered down to
	// an explicit allowlist, and secret-looking names are only allowed when the
	// user supplied them. Pass both layers: a key that arrives solely through
	// the workspace shared_env is in agentEnv but would be filtered out of the
	// Codex shell — silently, with no log — if only custom_env were authorized.
	userSuppliedEnv := mergeUserEnvLayers(agentWorkspaceEnv, agentCustomEnv)
...
// applyUserEnvLayer writes one layer of user-configured environment variables
// onto agentEnv, skipping daemon-blocklisted keys so no layer can override a
// daemon-set critical variable. layerName only labels the warning log.
//
// Callers apply layers lowest-precedence-first so later writes win. The order
// at task launch is workspace_env (the workspace-level shared_env, set once for
// every agent in the workspace) then custom_env (the agent's own vars), so
// custom_env overrides workspace_env for same-named keys. Covers the main agent
// process only; MCP servers get their env from their own mcp_config block.
func applyUserEnvLayer(agentEnv map[string]string, layerName string, env map[string]string, logger *slog.Logger) {
	for k, v := range env {
		if isBlockedEnvKey(k) {
			if logger != nil {
				logger.Warn(layerName+": blocked key skipped", "key", k)
			}
			continue
		}
		agentEnv[k] = v
...
// mergeUserEnvLayers returns workspaceEnv overlaid by customEnv — the effective
// user-supplied env for a task. Consumers that need to reason about the whole
// user-supplied set (Hermes external_dirs expansion, the Codex shell
// allowlist) must use this rather than custom_env alone, or keys that arrive
// only through the workspace shared_env are silently dropped by them while
// still being present in the agent process env.
func mergeUserEnvLayers(workspaceEnv, customEnv map[string]string) map[string]string {
	if len(workspaceEnv) == 0 {
		return customEnv
	}
	merged := make(map[string]string, len(workspaceEnv)+len(customEnv))
