...
func (h *Handler) DaemonRegister(w http.ResponseWriter, r *http.Request) {
	...
	for _, runtime := range req.Runtimes {
		...
		// someone flips the runtime back to private. Applies to both built-in
		// and custom-profile runtimes. Runs after inheritMachineCustomName
		// because that call reassigns registered from a fresh row.
		if registered.Visibility != "public" && h.isSharedRunner(r.Context(), registered.OwnerID) {
...
func daemonHeartbeatHTTPResponse(ack *protocol.DaemonHeartbeatAckPayload) map[string]any {
	...
	// server_capabilities is deliberately not mirrored: rpc-v1 is negotiated on
	// the WS ack only, and advertising it on the HTTP path would tell a daemon
	// it can use a transport this response did not come from.
	return resp
}
